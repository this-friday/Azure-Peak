...
/mob/living/carbon/human/proc/abandon_envoy()
	...
	var/style_choice = tgui_input_list(src, "How should they go out?", "ABANDON SHIP", style_options)
...
/mob/living/carbon/human/proc/communicate()
	...
	if(mind.warband_manager.disorder >= 8 && mind.special_role != ROLE_WARLORD) // warlord can always use Communicate
		to_chat(src, span_bold("I call, but no Carrier Zad heeds me. It's likely disturbed by the disorder in our Warband."))
		return

	if(mind.warband_manager.disorder >= 8 && mind.special_role == ROLE_WARLORD)
	...
	if(istype(loc.loc, /area/rogue/outdoors))
		...
		if(input_text)
			...
			if(do_after(src, 100, FALSE, src))
				...
				visible_message(span_boldred("The carrier zad takes flight, unburdened by any message!"))
				play_zad_ascend(src)
	else
		...
		for(var/obj/structure/fluff/warband/shortcut/warband_shortcut in SSwarbands.warband_machines)
			if(warband_shortcut.warband_ID == mind.warband_ID)
				if(warband_shortcut.disabled)
					...
					visible_message(span_bold("[src] slips somewhere beyond sight! Naught but a faint set of tracks remains..."))
					// as someone takes a shortcut, they leave behind a special set of tracks
					...
					if(departure_turf)
						...
						if(new_track)
							if(old_track && old_track != new_track)
								...
								SStracks.recycle_track(old_track)
							new_track.handle_creation(src)
					forceMove(get_turf(warband_shortcut))
				break
		return TRUE
...
/mob/living/carbon/human/proc/connect_warcamp()
	...
	if(!mind.warband_manager.warcamp_established) // if this is being done without a warcamp, we check if there's a free space.
		...
		if(!found_slot)
			...
			if(mind.special_role == ROLE_WARLORD) // if they're a warlord, assume they're a deserter and let them place a Recruitment Point
...
/mob/living/carbon/human/proc/accept_kick()
	...
	var/exile_choice = tgui_input_list(src, "Who should I settle?", "EXILE", mind.unresolved_exile_names)
	if(exile_choice)
	...
	var/readycheck = tgui_input_list(src, "Will I endorse the exile of [target.real_name]?", "EXILE", list("Defy Exile (Keep as Personal Associate)", "Accept (Cut Ties)", "Cancel"))
...
/mob/living/carbon/human/proc/enlighten()
	...
	var/mob/living/carbon/human/chosen_target = tgui_input_list(src, "Who shall receive enlightenment?", "ENLIGHTENMENT", nearby_targets)
	if(!chosen_target)
		return FALSE
	...
	var/target_choice = tgui_alert(chosen_target, "Do I accept [src]'s offer of enlightenment?", "ENLIGHTENMENT", list("I ACCEPT", "I REFUSE"))
...
/mob/living/carbon/human/proc/apply_enlightenment(mob/living/carbon/human/target)
	...
	var/had_other_patron = target.patron?.type != patron.type // captured before the conversion overwrites it
	...
	if(!HAS_TRAIT(target, TRAIT_DNR) && had_other_patron && patron.type != /datum/patron/old_god) // if they had a different patron, they get the DNR trait
